...
const {handleGetNodeAddresses,handleInitConnector,handleConnect,handleExecuteTransaction,handleDisconnectSessions,handleHederaSignTransaction,signers} = useHederaTransactions();
    /**
      ...
      <main className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-[#2e026d] to-[#15162c] text-white">
        ...
        <section>
          ...
          <fieldset>
            ...
            <button onClick={()=>modalWrapper(async ()=>handleGetNodeAddresses())}>
                ...
                //   if (!selectedSigner) throw new Error('Selected signer is required')
                  switch (selectedTransactionMethod) {
                    case 'hedera_executeTransaction':
                      return handleExecuteTransaction(signerPrivateKey,  AccountId(signerAccount),AccountId(receiver) ,amount)
                    case 'hedera_signAndExecuteTransaction':
                      return handleHederaSignAndExecuteTransaction()
                    case 'hedera_signTransaction':
                      return handleHederaSignTransaction(AccountId(receiver) ,amount)
                  }
                })
              }}
            >
